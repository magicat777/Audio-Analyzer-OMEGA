...
from omega4.panels.pitch_detection import CepstralAnalyzer
    ...
    def analyze_harmonics_from_fundamental(
        ...
        for n in range(1, max_harmonic + 1):
            ...
            freq_tolerance = fundamental_freq * 0.05
    ...
    def combine_pitch_estimates(self, signal: np.ndarray, cepstrum: np.ndarray) -> Dict[str, Any]:
        ...
        if cepstral_conf > 0.2 and self.min_pitch <= cepstral_pitch <= self.max_pitch:
            estimates.append((cepstral_pitch, cepstral_conf * 0.8))
        ...
        if yin_conf > 0.4 and self.min_pitch <= yin_pitch <= self.max_pitch:
            estimates.append((yin_pitch, yin_conf * 1.0))
    ...
    def calculate_pitch_stability(self) -> float:
        ...
        if mean_pitch > 0:
            ...
            stability = max(0.0, 1.0 - relative_std * 10)
        else:
            stability = 0.0
...
class ProfessionalLiveAudioAnalyzer:
    ...
    def __init__(self, width=2000, height=1080, bars=BARS_DEFAULT, source_name=None):
        ...
        self.gain_history = deque(maxlen=300)
        ...
        self.ring_buffer = np.zeros(FFT_SIZE_BASE * 4, dtype=np.float32)
        self.buffer_pos = 0

        self.freqs = np.fft.rfftfreq(FFT_SIZE_BASE, 1 / SAMPLE_RATE)
        
        self.adaptive_allocation_enabled = False
        ...
        self.midrange_boost_factor = 2.0
        ...
        self.quality_mode_hold_time = 10.0
        self.last_quality_switch_time = 0.0
        
        
        self.show_vu_meters = True
        ...
        self.peak_decay_rate = 0.85
        self.peak_hold_time = 3.0
        self.peak_timestamps = np.zeros(self.bars, dtype=np.float64)
        
        self.bass_peak_hold = {}
        self.bass_peak_timestamps = {}
        ...
        self.show_pitch_detection = False
        self.show_chromagram = False
        self.show_genre_classification = False
        self.show_help = False

        self.meter_panel_width = 280
        self.bass_zoom_height = 200
        self.left_border_width = 100
        self.vu_meter_width = 190
    ...
    def draw_organized_header(self, header_height: int):
        ...
        if col3_x < self.width - 150:
            ...
            if active_features:
                features_text = f"🔧 Active: {' • '.join(active_features[:3])}"
    ...
    def _create_enhanced_band_mapping(self):
        ...
        fft_bin_width = self.freqs[1] - self.freqs[0]
        min_fft_freq = fft_bin_width
        ...
        sub_bass_bars = int(total_bars * 0.08)
        bass_bars = int(total_bars * 0.17)
        low_mid_bars = int(total_bars * 0.15)
        mid_bars = int(total_bars * 0.25)
        high_mid_bars = int(total_bars * 0.20)
    ...
    def process_frame(self):
        ...
        if (self.show_voice_info or self.show_formants) and self.frame_counter % analysis_skip == 0:
            ...
            voice_info = getattr(self, 'voice_info', self.voice_info)
        ...
        if self.frame_counter % (analysis_skip + 1) == 0:
            ...
            drum_info = getattr(self, 'drum_info', self.drum_info)
        ...
        self.room_modes = room_modes
        self.pitch_info = pitch_info
        self.chromagram_info = chromagram_info
    ...
    def _vectorized_smoothing(self, band_values: np.ndarray, drum_info: Dict, voice_info: Dict):
        ...
        voice_attack = 0.95 if has_voice else 0.85
    ...
    def get_frequency_range_for_bar(self, bar_index: int) -> Tuple[float, float]:
        ...
        if bar_index < len(self.band_indices):
            ...
            if len(indices) > 0:
                ...
                if len(indices) == 1:
                    if indices[0] > 0 and bar_index > 0:
    ...
    def update_peak_holds(self):
        ...
        for i in range(num_bars):
            if self.bar_heights[i] > self.peak_hold_bars[i] + 0.02:
                self.peak_hold_bars[i] = self.bar_heights[i]
                self.peak_timestamps[i] = current_time
            
            if self.peak_timestamps[i] > 0:
    ...
    def draw_sub_bass_indicator(self):
        ...
        meter_height = 200
        meter_width = 25
        x_pos = (self.left_border_width - meter_width) // 2
    ...
    def detect_and_display_room_modes(self):
        ...
        for i in range(min(150, self.bars)):
            freq_start, freq_end = self.get_frequency_range_for_bar(i)
            center_freq = (freq_start + freq_end) / 2
    ...
    def draw_voice_info(self):
        ...
        if hasattr(self.cepstral_analyzer, 'pitch_history') and len(self.cepstral_analyzer.pitch_history) > 10:
            ...
            graph_y = overlay_y + overlay_height - graph_height - 20
            ...
            if history:
                ...
                if valid_pitches:
                    ...
                    points = []
                    for i, pitch in enumerate(history[-50:]):
                        if pitch > 0:
    ...
    def calculate_spectral_tilt(self) -> float:
        ...
        if low_energy > 0 and high_energy > 0:
            return 20 * np.log10(high_energy / low_energy) / 4.3
        return 0.0
    ...
    def print_debug_output(self):
        ...
        for pos in range(width):
            ...
            target_freq = 20 * (20000 / 20) ** freq_position
        ...
        scale_height = 35
        visualization_height = height - 40 - scale_height

        if hasattr(self, 'bass_bar_values'):
            total_width = width - 20
            bar_width = total_width / self.bass_detail_bars
            max_height = visualization_height
            current_time = time.time()
            bar_bottom = y + height - scale_height - 5
            ...
            for j in range(self.bass_detail_bars):
                ...
                freq_position = max(0.0, min(1.0, freq_position))
    ...
    def draw_frame(self):
        ...
        if self.show_genre_classification:
            ...
            base_y = int((280 + 10 + 200 + 10) * self.ui_scale)
            if self.show_pitch_detection:
                base_y += int((320 + 10) * self.ui_scale)
            if self.show_chromagram:
                base_y += int((260 + 10) * self.ui_scale)
    ...
    def run(self):
        ...
        try:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_ESCAPE:
                            ...
                            self.resize_window(1400, 900)
                        elif event.key == pygame.K_2:
                            self.resize_window(1800, 1000)
                        elif event.key == pygame.K_3:
                            self.resize_window(2200, 1200)
                        elif event.key == pygame.K_4:
                            self.resize_window(2600, 1400)
                        elif event.key == pygame.K_5:
                            self.resize_window(3000, 1600)
                        elif event.key == pygame.K_6:
                            self.resize_window(3400, 1800)
                        elif event.key == pygame.K_7:
                            self.resize_window(1920, 1080)
                        elif event.key == pygame.K_8:
                            self.resize_window(2560, 1440)
                        elif event.key == pygame.K_9:
                            self.resize_window(3840, 2160)
                            ...
                            self.input_gain *= 1.5
                            self.input_gain = min(self.input_gain, 16.0)
                            gain_db = 20 * np.log10(self.input_gain)
                            print(f"🔊 Input gain: +{gain_db:.1f}dB")
                        elif event.key == pygame.K_MINUS:
                            self.input_gain /= 1.5
                            self.input_gain = max(self.input_gain, 0.25)
                            gain_db = 20 * np.log10(self.input_gain)
                            print(f"🔊 Input gain: {gain_db:+.1f}dB")
                        elif event.key == pygame.K_0:
                            self.input_gain = 4.0
            ...
            self.bass_processing_queue.put(None)
