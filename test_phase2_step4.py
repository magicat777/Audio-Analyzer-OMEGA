#!/usr/bin/env python3
"""Test Phase 2 Step 4: Grid and labels drawing"""
...
# Test display interface with grid
try:
...
# Test drawing with grid
try:
    display.clear_screen()
    display.draw_grid_and_labels(grid_params, ui_scale=1.0)
    display.draw_spectrum_bars(spectrum_data, vis_params)
    pygame.display.flip()
    print("✓ Grid and labels drawn successfully")
    print("✓ Spectrum bars drawn over grid")
    
    # Keep window open to verify
    time.sleep(2)
    
except Exception as e:
    print(f"✗ Failed to draw grid: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

print("\nPhase 2 Step 4 Complete: Grid and labels working!")
pygame.quit()