...
# Import array pooling for performance
from ..optimization import get_array, return_array, with_array
...
class MultiResolutionFFT:
    ...
    def combine_results_optimized(self, results: Dict[int, FFTResult], 
        ...
        try:
            ...
            # Use array pool for temporary arrays
            combined_magnitude = get_array((target_bins,), dtype=np.float32)
            weight_sum = get_array((target_bins,), dtype=np.float32)
            ...
            # Make a copy before returning arrays to pool
            result_magnitude = combined_magnitude.copy()
            
            # Return temporary arrays to pool
            return_array(weight_sum)
            return_array(combined_magnitude)
            
            return result_magnitude, target_freqs
