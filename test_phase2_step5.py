#!/usr/bin/env python3
"""Test Phase 2 Step 5: Frequency scale integration"""
...
# Test display interface with frequency scale
try:
...
# Test drawing with frequency scale
try:
    display.clear_screen()
    display.draw_grid_and_labels(grid_params, ui_scale=1.0)
    display.draw_spectrum_bars(spectrum_data, vis_params)
    display.draw_frequency_scale(freq_params, None, ui_scale=1.0)
    pygame.display.flip()
    print("✓ Frequency scale drawn successfully")
    print("✓ Complete display stack working")
    
    # Keep window open to verify
    time.sleep(2)
    
except Exception as e:
    print(f"✗ Failed to draw frequency scale: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)

print("\nPhase 2 Step 5 Complete: Frequency scale integrated!")
pygame.quit()